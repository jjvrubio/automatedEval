...
def read_json_file(file_path: str) -> List[str]:
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        if "references" in data and isinstance(data["references"], list):
            ...
            raise ValueError(
                "El archivo JSON no contiene la clave 'references' o no es una lista."
            )
...
def analyze_reference(reference: str) -> dict:
    ...
    has_title = any(
        word.istitle() for word in reference.split()
...
def generate_markdown_report(references: List[str], report_path: str):
    ...
    report_lines.append(
        "Este informe detalla el estado de cumplimiento de las referencias en formato APA 7.\n"
    )
    ...
    for i, reference in enumerate(references, start=1):
        ...
        report_lines.append(
            f"**Cumplimiento APA 7:** {'✔️ Cumple' if analysis['is_compliant'] else '❌ No cumple'}\n"
        )
        report_lines.append("\n")
