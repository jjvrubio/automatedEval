...
try:
    from referencias_validator import (
        ExtractorReferencias,
        ValidadorReferencias,
        GeneradorInformes,
        EstiloCita,
        ReferenciaAnalizada,
        InformeValidacion,
    )
    from utils_referencias import (
        DetectorPatrones,
        LimpiadorTexto,
        AnalizadorComponentes,
    )
...
def ejemplo_validacion_referencias_texto():
    ...
    puntuacion_promedio = sum(ref.puntuacion for ref in referencias_analizadas) / len(
        referencias_analizadas
    )
    ...
    print(
        f"   Porcentaje de cumplimiento: {(referencias_validas / len(referencias_ejemplo) * 100):.1f}%"
    )
...
def main():
    ...
    try:
        ...
        print(
            "   python referencias_validator.py --archivo tu_tesis.docx --estilo ieee"
        )
