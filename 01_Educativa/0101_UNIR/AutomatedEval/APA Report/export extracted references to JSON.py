...
def extract_references_from_pdf(pdf_path):
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            ref_positions = [
                i for i, line in enumerate(text.splitlines()) if "REFERENCIAS" in line
            ]
            ...
            if not extracted_references:
                ...
                save_json_file(extracted_references)

    except Exception as e:
        print(f"Error during extraction: {e}")


def save_json_file(references):
    """Open a macOS save panel to save references as JSON."""
    ...
    if panel.runModal() == 1:
        ...
        if file_url:
            ...
            print(f"References successfully saved to {file_path}")
        else:
