    ...
    try:
        ...
        if is_onedrive_path(path):
            ...
            if Path(fpctl).exists():
                ...
                logger.info(
                    "'fileproviderctl' no disponible. Intento de lectura directa."
                )
        # intento final
        try:
            with open(path, "rb") as f:
...
def seleccionar_archivo_referencias(logger: logging.Logger) -> Optional[str]:
    ...
    try:
        ...
        panel.setMessage_(
            "Elige el archivo PDF o DOCX cuyas referencias quieres validar"
        )
        if panel.runModal() == 1:
        ...
        while True:
            try:
                ...
                if 0 <= indice < len(archivos_disponibles):
                    ...
                    archivo = input(
                        "📄 Introduce la ruta completa del archivo: "
                    ).strip()
                    return archivo if archivo else None
                else:
                    print(
                        f"❌ Selección inválida. Debe ser entre 1 y {len(archivos_disponibles) + 1}"
                    )
            except ValueError:
...
class ExtractorReferencias:
    ...
    def extraer_desde_pdf(ruta_archivo: str) -> Tuple[List[str], List[str]]:
        ...
        try:
            with pdfplumber.open(ruta_archivo) as pdf:
                ...
                patrones_detectados = (
                    ExtractorReferencias._detectar_patrones_repetitivos(pdf)
                )
                ...
                for pagina in pdf.pages:
                    ...
                    if texto_pagina:
                        ...
                        texto_limpio = ExtractorReferencias._limpiar_texto(
                            texto_pagina, patrones_detectados
                        )
                        texto_completo += texto_limpio + "\n"

                # Extraer referencias de la sección correspondiente
                referencias = ExtractorReferencias._extraer_referencias_del_texto(
                    texto_completo
                )
    ...
    def _detectar_patrones_repetitivos(pdf) -> List[str]:
        ...
        patrones_repetitivos = [
            patron
            for patron, count in patrones_texto.items()
    ...
    def _limpiar_texto(texto: str, patrones_a_remover: List[str]) -> str:
        ...
        texto_limpio = re.sub(
            r"^(Page|Página) \d+$", "", texto_limpio, flags=re.MULTILINE
        )
    ...
    def _extraer_referencias_del_texto(texto: str) -> List[str]:
        ...
        if inicio_referencias != -1:
            ...
            for parrafo in parrafos:
                ...
                if (
                    len(parrafo_limpio) > 50  # Longitud mínima
                    and "." in parrafo_limpio  # Debe tener puntos
                    and not parrafo_limpio.lower().startswith(
                        ("referencias", "bibliografía", "references")
                    )  # No es encabezado
                    and not re.match(r"^\d+\s*$", parrafo_limpio)
                ):  # No es solo número
...
class ValidadorReferencias:
    ...
    def _verificar_titulo(self, referencia: str) -> bool:
        ...
        return bool(
            re.search(r'["\'][^"\']{10,}["\']', referencia)
            or re.search(r"[A-Z][^.]{10,}\.", referencia)
        )
        ...
        if informe.recomendaciones_generales:
            lineas.extend(
                [
                    ...
                    *[
                        f"- {recomendacion}"
                        for recomendacion in informe.recomendaciones_generales
                    ],
                    "",
                ]
            )
...
def modo_interactivo():
    ...
    while True:
        try:
            seleccion_estilo = input(
                "\n📝 Selecciona estilo (1-3) [por defecto: APA]: "
            ).strip()
    ...
    while True:
        try:
            seleccion_formato = input(
                "\n📊 Selecciona formato (1-2) [por defecto: Markdown]: "
            ).strip()
    ...
    return {"archivo": archivo, "estilo": estilo, "formato": formato, "salida": None}
...
Ejemplos de uso:
  ...
  python referencias_validator.py --interactivo
    ...
    parser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Modo interactivo para seleccionar archivo y opciones",
    )
    ...
    if not Path(archivo).exists():
        ...
        if archivos_disponibles:
            ...
            print('   python referencias_validator.py --archivo "nombre_archivo.pdf"')
    ...
    try:
        ...
        if extension == ".pdf":
            if not PDF_AVAILABLE:
                print(
                    "❌ Error: pdfplumber no está instalado. Ejecutar: pip install pdfplumber"
                )
                sys.exit(1)
        ...
        if referencias_validas / len(referencias) < 0.8:
            recomendaciones.append(
                "Revisar formato general de las referencias según el estilo seleccionado"
            )
        ...
        puntuacion_promedio = sum(
            ref.puntuacion for ref in referencias_analizadas
        ) / len(referencias_analizadas)
        ...
        print(
            f"   ❌ Referencias con errores: {len(referencias) - referencias_validas}"
        )
        print(f"   📈 Puntuación promedio: {puntuacion_promedio:.2f}/1.00")
        print(
            f"   📋 Cumplimiento: {(referencias_validas / len(referencias) * 100):.1f}%"
        )
