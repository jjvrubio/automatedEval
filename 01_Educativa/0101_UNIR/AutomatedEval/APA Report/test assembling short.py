...
class PDFAnalyzer:
    ...
    def merge_lines_with_positions(self, y_tolerance=2):
        ...
        if not self.text_elements:
            raise ValueError(
                "No text elements found. Run `extract_text_with_positions` first."
            )
    ...
    def filter_repeated_lines(self, min_repetition=3):
        ...
        if not self.merged_lines:
            raise ValueError(
                "No merged lines found. Run `merge_lines_with_positions` first."
            )
        ...
        self.merged_lines = [
            line for line in self.merged_lines if line_counts[line] <= min_repetition
        ]
    ...
    def analyze_text(self):
        ...
        last_10_paragraphs = (
            self.merged_lines[-10:] if paragraph_count >= 10 else self.merged_lines
        )
