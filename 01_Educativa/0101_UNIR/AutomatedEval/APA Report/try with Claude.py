def clean_and_prepare_references(raw_text, debug=True):
    ...
    start_pattern = re.compile(
        r"(?i)^Referencias bibliográficas(?![.\s\d])", re.MULTILINE
    )
    ...
    for match in start_pattern.finditer(cleaned_text):
        line_end = cleaned_text[
            match.end() : cleaned_text.find("\n", match.end())
        ].strip()
        if not re.match(r"[.\s\d]", line_end):
            ...
            debug_info["marker_debug"]["start_marker_line"] = cleaned_text[
                match.start() : cleaned_text.find("\n", match.start())
            ].strip()
            break
    ...
    if end_match:
        ...
        debug_info["marker_debug"]["end_marker_line"] = cleaned_text[
            end_match.start() : cleaned_text.find("\n", end_match.start())
        ].strip()
    else:
    ...
    if debug:
        with open("references_debug.md", "w", encoding="utf-8") as f:
            ...
            f.write("## Marker Pattern Analysis\n")
            f.write(
                f"✅ Start marker found at position {debug_info['marker_debug']['start_marker_position']}\n"
            )
            f.write(f"- Line: `{debug_info['marker_debug']['start_marker_line']}`\n\n")

            if debug_info["marker_debug"]["end_marker_found"]:
                f.write(
                    f"✅ End marker found at position {debug_info['marker_debug']['end_marker_position']}\n"
                )
