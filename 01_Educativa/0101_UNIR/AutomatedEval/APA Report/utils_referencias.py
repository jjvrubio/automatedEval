...
class DetectorPatrones:
    ...
    @staticmethod
    def extraer_seccion_referencias(
        texto: str, idioma: str = "auto"
    ) -> Tuple[str, int]:
        """
    ...
    def identificar_tipo_referencia(referencia: str) -> str:
        ...
        if any(
            patron in ref_lower
            for patron in ["vol.", "volume", "núm.", "number", "pp.", "pages"]
        ):
        ...
        if any(
            patron in ref_lower
            for patron in ["editorial", "press", "publisher", "ediciones"]
        ):
            if not any(
                patron in ref_lower for patron in ["journal", "revista", "vol."]
            ):
                return "libro"
        ...
        if any(
            patron in ref_lower
            for patron in ["en:", "in:", "editor", "eds.", "compilador"]
        ):
            return "capitulo"
        ...
        if any(
            patron in ref_lower
            for patron in ["tesis", "thesis", "dissertat", "maestría", "doctorado"]
        ):
            return "tesis"
        ...
        if any(
            patron in ref_lower
            for patron in ["http", "www.", "recuperado", "accessed", "available"]
        ):
            return "web"
...
class LimpiadorTexto:
    ...
    def remover_patrones_repetitivos(texto: str, patrones: List[str]) -> str:
        ...
        for patron in patrones:
            ...
            texto_limpio = re.sub(
                patron_escapado, "", texto_limpio, flags=re.IGNORECASE
            )
...
class AnalizadorComponentes:
    ...
    @staticmethod
    def extraer_titulo(
        referencia: str, tipo_publicacion: str = "articulo"
    ) -> Optional[str]:
        """
...
class ValidadorFormato:
    ...
    def validar_formato_apa(referencia: str) -> Dict[str, bool]:
        ...
        """
        validaciones = {
            "autor_formato_correcto": bool(
                re.search(r"[A-ZÁÉÍÓÚÜ][a-záéíóúüñ]+,\s*[A-Z]\.", referencia)
            ),
            "anio_entre_parentesis": bool(re.search(r"\(\d{4}[a-z]?\)", referencia)),
            "titulo_capitalizado_correcto": ValidadorFormato._validar_capitalizacion_apa(
                referencia
            ),
    ...
    def validar_formato_ieee(referencia: str) -> Dict[str, bool]:
        ...
        """
        validaciones = {
            "autor_iniciales_primero": bool(
                re.search(r"[A-Z]\.\s*[A-ZÁÉÍÓÚÜ][a-záéíóúüñ]+", referencia)
            ),
            "titulo_entre_comillas": bool(re.search(r'"[^"]+",', referencia)),
            "abreviacion_revista": ValidadorFormato._validar_abreviacion_ieee(
                referencia
            ),
            "formato_volumen": bool(
                re.search(r"vol\.\s*\d+", referencia, re.IGNORECASE)
            ),
    ...
    def _validar_abreviacion_ieee(referencia: str) -> bool:
        ...
        abreviaciones_ieee = [r"IEEE\s+Trans\.", r"Proc\.", r"J\.", r"Lett\.", r"Rev\."]
...
def generar_estadisticas_referencias(referencias: List[str]) -> Dict:
    ...
    for referencia in referencias:
        ...
        estadisticas["tipos_detectados"][tipo] = (
            estadisticas["tipos_detectados"].get(tipo, 0) + 1
        )
