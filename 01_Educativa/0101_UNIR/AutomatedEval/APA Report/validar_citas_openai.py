...
def crear_cliente_openai():
    ...
    if not api_key:
        raise RuntimeError(
            "ERROR: No se encontró la variable de entorno MI_CLAVE_API_OPENAI o OPENAI_API_KEY"
        )
...
def extraer_referencias_automaticamente(compat, client, texto_completo):
    ...
    for seccion in posibles_secciones:
        ...
        if pos != -1:
            ...
            logger.info(
                f"Sección de referencias encontrada: '{seccion}' en posición {pos}"
            )
            break
...
"""
    ...
    try:
        if compat == "v1":
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un experto en extraer referencias bibliográficas de textos académicos.",
                    },
        ...
        if referencias_extraidas:
            referencias_lista = [
                ref.strip()
                for ref in referencias_extraidas.split("\n")
                if ref.strip() and len(ref.strip()) > 20
            ]  # Filtrar líneas muy cortas

            logger.info(
                f"Referencias extraídas automáticamente: {len(referencias_lista)}"
            )
        ...
        if compat == "v1":
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un experto en normas APA 7 y análisis bibliográfico integral.",
                    },
                    {"role": "user", "content": prompt},
                ],
...
def extraer_partes_relevantes_para_citas(texto_completo, max_chars=25000):
    ...
    logger.info(
        f"Texto optimizado: {len(texto_optimizado)} caracteres de {len(texto_completo)} originales"
    )
        ...
        if compat == "v1":
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un experto en normas APA 7 y revisión académica.",
                    },
                    {"role": "user", "content": prompt},
                ],
                max_tokens=1500,
...
def main():
    try:
        ...
        tfm_file = select_file(
            ["pdf", "docx", "txt"], "Selecciona el archivo del TFM (PDF, DOCX o TXT)"
        )
        if not tfm_file:
        ...
        if opcion == "1":
            ...
            ref_file = select_file(
                ["json"], "Selecciona el archivo de referencias APA (JSON)"
            )
            if not ref_file:
            ...
            try:
                ...
                logger.info(
                    f"Referencias cargadas desde JSON: {len(referencias)} elementos"
                )
            except Exception as e:
