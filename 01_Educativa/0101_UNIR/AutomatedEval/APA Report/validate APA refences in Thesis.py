...
def identify_head_footer_pattern(pdf):
    ...
    head_footer_patterns = [
        pattern for pattern, count in text_patterns.items() if count > 0.8 * total_pages
    ]
...
def filter_head_footer_patterns(text, patterns):
    ...
    filtered_text = "\n".join(
        line
        for line in lines
        if line.strip() not in patterns and not is_page_number(line)
    )
...
def check_apa7_compliance(references):
    ...
    for i, ref in enumerate(references, 1):
        if re.match(apa_pattern, ref):
            report.append({"Reference": i, "Status": "✅ APA 7 Compliant", "Text": ref})
        else:
            report.append(
                {"Reference": i, "Status": "❌ Not APA 7 Compliant", "Text": ref}
            )
