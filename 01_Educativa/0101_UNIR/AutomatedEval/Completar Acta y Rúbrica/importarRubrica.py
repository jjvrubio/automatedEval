        ...
        try:
            ...
            for opt in select.options:
                if valor_buscado.strip().lower() == opt.text.strip().lower():
                    ...
                    trazar(
                        f"✅ '{valor_buscado}' seleccionado en <select #{i}> con texto exacto: '{opt.text}'"
                    )
                    with open(
                        f"seleccion_{valor_buscado.replace(' ', '_')}_safari.html",
                        "w",
                        encoding="utf-8",
                    ) as f:
            ...
            continue
    trazar(
        f"❌ No se pudo seleccionar '{valor_buscado}' en ningún <select>",
    )
    return False
