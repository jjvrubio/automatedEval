...
def construir_prompt(
    criterio: str, instrucciones: str, texto_tfm: str, doc_hash: str, phrases: List[str]
) -> str:
...
def evaluar_tfm_minimal(
    ...
    for _, row in rubrica_df.iterrows():
        ...
        try:
            ...
            try:
                nivel_idx = (
                    int(nivel_elegido) - 1 if nivel_elegido is not None else None
                )
            except Exception:
                nivel_idx = None
            if (
...
def _generar_markdown(
    resultados: List[Dict[str, Any]], preguntas: Optional[List[Dict[str, Any]]] = None
) -> str:
...
def generar_preguntas_clarificacion(
    texto_tfm: str,
    resultados: List[Dict[str, Any]],
    config: Dict[str, Any],
    logger: Any,
    ...
    numero = (
        analisis_cfg.get("numero_preguntas_exactas")
        or analisis_cfg.get("numero_preguntas")
        or 3
    )
    try:
    ...
    doc_hash = hashlib.md5(texto_tfm.encode("utf-8")).hexdigest()
    if plantilla_text:
        prompt = plantilla_text.format(
            numero_preguntas=numero, numero_preguntas_exactas=numero
        )
        prompt = (
    ...
    questions = (
        ...
        or parsed.get("questions_list")
        or parsed.get("questions")
    )
    if not isinstance(questions, list):
        raise RuntimeError(
            "La respuesta del modelo no contiene una lista 'questions' en JSON"
        )
    ...
    if len(uniq) < numero:
        raise RuntimeError(
            f"Se generaron {len(uniq)} preguntas únicas, se esperaban {numero}"
        )
    return uniq[:numero]
...
def main() -> int:
    ...
    csv_path, json_path, md_path = paths_para_exportar(
        ruta, time.strftime("%Y%m%d_%H%M%S"), config
    )
