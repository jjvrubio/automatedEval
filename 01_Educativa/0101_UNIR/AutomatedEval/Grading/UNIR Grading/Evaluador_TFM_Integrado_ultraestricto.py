...
def _should_apply_rulepack(criterio: str, pack_cfg: Dict[str, Any]) -> bool:
    ...
    Soporta:
      - pack_cfg['apply_to_criterios']: lista de substrings o nombres exactos
      - heurística por defecto: criterios tipo 'introducción/objetivos/justificación' y 'proyecto'
    """
    crit = (criterio or "").strip()
    crit_lower = crit.lower()

    apply_list = pack_cfg.get("apply_to_criterios")
    if isinstance(apply_list, (list, tuple)) and apply_list:
        for item in apply_list:
            if not isinstance(item, str) or not item.strip():
                continue
            token = item.strip().lower()
            if token in crit_lower:
                return True
        return False

    # Heurística por defecto (conservadora): análisis externo suele ser crítico aquí.
    return any(
        key in crit_lower
        for key in [
            "introdu",
...
def _resolve_rulepacks(cfg: Dict[str, Any], base_dir: Path) -> None:
    """
    Resuelve rulepacks declarados en cfg["evaluador"]["rulepacks"].
    Carga rules_file y lo inyecta como cfg["evaluador"]["rulepacks"][name]["rules"].
    """
    ...
    for name, pack in rulepacks.items():
        ...
        rules_file = pack.get("rules_file")
        if not rules_file:
            continue
        ...
        pack["resolved_rules_path"] = str(rules_path)
        pack["rules"] = _load_yaml_file(rules_path)
...
def cargar_config_yaml() -> Dict[str, Any]:
    ...
    rp = cfg.get("evaluador", {}).get("rulepacks", {}).get("analisis_externo", {})
    print("Rulepack enabled:", rp.get("enabled"))
    print("Rules path:", rp.get("resolved_rules_path"))
    print("Rules loaded:", isinstance(rp.get("rules"), dict) and len(rp.get("rules", {})) > 0)

    # Resolver rulepacks (si existen) sin acoplarse al orden del YAML
    try:
        _resolve_rulepacks(cfg, TFM_PACKAGE)
    cfg.setdefault("runtime", {})["rulepacks_resolved"] = True
    except Exception as e:
        cfg.setdefault("runtime", {})["rulepacks_resolved"] = False
        cfg["runtime"]["rulepacks_error"] = repr(e)
    return cfg
...
def evaluar_tfm_minimal(
    ...
    for _, row in rubrica_df.iterrows():
        ...
        extra_metodologia, metodologias_relacionadas = _extra_instrucciones_metodologicas(
            criterio,
            config,
        ...
        # --- Rulepack: análisis externo (gates/caps) -------------------------
        rulepack_addendum = ""
        rulepack_report: Optional[Dict[str, Any]] = None
        rulepack_cap_level: int = 4
        pack_cfg = (
            (config.get("evaluador") or {}).get("rulepacks", {}).get("analisis_externo", {})
            if isinstance(config, dict)
            else {}
        )
        if (
            isinstance(pack_cfg, dict)
            and pack_cfg.get("enabled", False)
            and _should_apply_rulepack(criterio, pack_cfg)
        ):
            rules = pack_cfg.get("rules") if isinstance(pack_cfg.get("rules"), dict) else {}
            rulepack_report = _analisis_externo_gate_report(texto_categoria, rules)
            rulepack_cap_level = int(rulepack_report.get("cap_level", 4) or 4)
            rulepack_addendum = _format_rulepack_addendum(
                "analisis_externo", rulepack_report, pack_cfg
            )

        instrucciones_completas = instrucciones
        if extra_metodologia:
            instrucciones_completas = (
                instrucciones_completas
                + "\n"
                + "En la justificacion analiza explicitamente la cobertura metodologica: "
                + extra_metodologia
            )
        ...
        try:
            ...
            # Aplicar cap determinista del rulepack (si corresponde)
            if rulepack_report is not None:
                try:
                    parsed["rulepack_analisis_externo"] = json.dumps(
                        rulepack_report, ensure_ascii=False
                    )
                except Exception:
                    parsed["rulepack_analisis_externo"] = str(rulepack_report)
                parsed["rulepack_analisis_externo_cap_level"] = rulepack_cap_level
                _enforce_cap_level(parsed, rulepack_cap_level, rulepack_report)
