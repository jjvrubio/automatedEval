...
def seleccionar_archivo_tfm(logger: logging.Logger) -> Optional[str]:
    ...
    if panel.runModal() == NSModalResponseOK:
        ...
        logger.info(
            f"🔍 TAMAÑO: {os.path.getsize(path) if os.path.exists(path) else 'NO EXISTE'} bytes"
        )
...
def obtener_respuesta_openai(
    prompt: str, config: Dict[str, Any], logger: logging.Logger
) -> str:
    """
    ...
    try:
        ...
        logger.info(
            f"🤖 Llamando OpenAI - Modelo: {modelo}, Temp: {temperatura}, Tokens: {max_tokens}"
        )
...
def cargar_rubrica(
    etiquetas: List[str], config: Dict[str, Any], logger: logging.Logger
) -> Optional[Any]:
    """
...
def evaluar_tfm_con_yaml(
    texto_tfm: str, rubrica_df: Any, config: Dict[str, Any], logger: logging.Logger
) -> List[Dict[str, Any]]:
    """
...
"""
            ...
            if respuesta:
                try:
                    ...
                    logger.info(
                        f"✅ Evaluado: {criterio} -> Nivel {resultado.get('nivel', 'N/A')}"
                    )
...
def exportar_resultados_yaml(
    resultados: List[Dict[str, Any]], config: Dict[str, Any], logger: logging.Logger
) -> None:
    """
