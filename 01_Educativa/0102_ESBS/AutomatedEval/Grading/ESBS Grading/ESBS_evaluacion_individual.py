...
def dividir_tfm_en_bloques_por_paginas(file_path: str) -> dict:
    with open(file_path, "rb") as f:
        ...
        for i, n in enumerate(tercios):
            texto = "".join(
                reader.pages[p].extract_text() or "" for p in range(start, start + n)
            )
...
def construir_prompt_desde_config(
    instrucciones: str,
    rubrica_md: str,
    ...
    instrucciones_seccion_str = "\n".join(
        f"- {line}" for line in instrucciones_por_seccion
    )
...
def main():
    ...
    base_path = Path(
        "/Users/juanjo/Documents/Personal/JJVR/automatizaciones/automatedEval/Grading/ESBS Grading/esbs_grader"
    )
    ...
    for nombre, texto in bloques.items():
        ...
        print(
            f"[DEBUG] Respuesta completa del LLM para el bloque '{nombre}':\n{evaluacion}\n{'=' * 80}"
        )
        ...
        if tabla_encontrada:
            print(
                f"[DEBUG] Tabla encontrada en bloque '{nombre}':\n{tabla_encontrada.group(0)}"
            )
            ...
            print(
                f"[ADVERTENCIA] No se encontró tabla de puntuaciones en el bloque '{nombre}'."
            )
            puntuaciones_bloques.append({})
