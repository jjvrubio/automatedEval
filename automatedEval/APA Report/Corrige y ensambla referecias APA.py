...
def debug_output(filename, data, input_file_path):
    ...
    output_file_path = os.path.join(input_dir, f"{input_filename}_{filename}_Referencias_APA.json")
            ...
            if file_path.endswith(".json"):
                with open(file_path, "r", encoding="utf-8") as file:
                    ...
                    if isinstance(data, list):
                        return [ref.strip() for ref in data if isinstance(ref, str) and ref.strip()], file_path
                    else:
                        print("The selected JSON file does not contain a list of references.")
                        return [], None
            elif file_path.endswith(".txt"):
                with open(file_path, "r", encoding="utf-8") as file:
                    return [line.strip() for line in file.readlines() if line.strip()], file_path
            else:
...
def detect_embedded_references(references, input_file_path):
    ...
    debug_output("refined_detect_embedded_references", {
...
def identify_offending_lines(references, input_file_path):
    ...
    debug_output("identify_offending_lines", [references[i] for i in offending_indices], input_file_path)
...
def check_apa_format(references, input_file_path):
    ...
    input_dir = os.path.dirname(input_file_path)
    input_filename = os.path.splitext(os.path.basename(input_file_path))[0]
    output_file_path = os.path.join(input_dir, f"{input_filename}_Referencias en formato.json")
...
def main():
    ...
    # Step 1A: Detect embedded references
    split_references, embedded_indices = detect_embedded_references(references, input_file_path)
    offending_indices = identify_offending_lines(split_references, input_file_path)
    merged_references = merge_offending_lines(split_references, offending_indices, input_file_path)
