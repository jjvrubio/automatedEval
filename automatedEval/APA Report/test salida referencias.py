    ...
    for i, line in enumerate(references):
        ...
        if len(matches) > 1:  # Multiple references found in the same line
            ...
            for j, match in enumerate(matches):
                ...
                if j == 0:  # First match
                    split_references.append(line[:start].strip() + line[start:end].strip())
                else:  # Subsequent matches
                    split_references.append(line[last_start:start].strip() + line[start:end].strip())

                last_start = end

            if last_start < len(line):
                split_references.append(line[last_start:].strip())  # Remaining text after the last match
        else:
            split_references.append(line)

    # Debug output for embedded references and split results
    debug_output("detected_embedded_references.json", {"embedded_indices": embedded_indices, "split_references": split_references})
