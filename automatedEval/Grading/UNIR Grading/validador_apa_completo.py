...
class ValidadorAPACompleto:
    ...
    def validar_documento_completo(self, texto_completo: str, referencias: List[str]) -> ResultadoValidacionAPA:
        """
        ...
        refs_huerfanas, citas_huerfanas = self._identificar_huerfanas(citas, refs_analizadas)
        ...
        puntuacion = self._calcular_puntuacion_apa(citas, refs_analizadas, correspondencias, errores_formato)
        
        # 7. Generar recomendaciones
        recomendaciones = self._generar_recomendaciones(citas, refs_analizadas, errores_formato)
    ...
    def _analizar_referencias(self, referencias: List[str]) -> List[ReferenciaAnalizada]:
        ...
        for ref_texto in referencias:
            ...
            ref_analizada.autores, ref_analizada.año, ref_analizada.titulo = self._extraer_elementos_referencia(ref_texto)
    ...
    def _extraer_elementos_referencia(self, referencia: str) -> Tuple[List[str], str, str]:
        ...
        if año_match:
            ...
            autores = [autor.strip().rstrip('.') for autor in autores_raw if autor.strip()]
    ...
    def _verificar_correspondencias(self, citas: List[CitaEncontrada], referencias: List[ReferenciaAnalizada]) -> Dict[str, bool]:
        ...
        for cita in citas:
            ...
            for ref in referencias:
                if ref.autores and ref.año:
                    ...
                    if (autor_cita in autor_ref or autor_ref in autor_cita) and cita.año == ref.año:
                        correspondencias[clave_cita] = True
                        break
        
        return correspondencias
    
    def _identificar_huerfanas(self, citas: List[CitaEncontrada], referencias: List[ReferenciaAnalizada]) -> Tuple[List[str], List[str]]:
        ...
        for ref in referencias:
            if ref.autores and ref.año:
                encontrada = False
                for cita in citas:
                    if ref.año == cita.año and any(autor.lower() in ref.autores[0].lower() for autor in cita.autores):
                        encontrada = True
                        break
                if not encontrada:
        ...
        for cita in citas:
            encontrada = False
            for ref in referencias:
                if ref.año == cita.año and any(cita.autores[0].lower() in autor.lower() for autor in ref.autores):
                    encontrada = True
                    break
            if not encontrada:
    ...
    def _detectar_errores_formato(self, texto: str, referencias: List[str]) -> List[str]:
    ...
    def _calcular_puntuacion_apa(self, citas: List[CitaEncontrada], referencias: List[ReferenciaAnalizada], 
                                correspondencias: Dict[str, bool], errores: List[str]) -> int:
        ...
        if referencias:
            calidad_promedio = sum(ref.calidad_apa for ref in referencias) / len(referencias)
            puntuacion *= (calidad_promedio / 10)
        
        return max(0, int(puntuacion))
    
    def _generar_recomendaciones(self, citas: List[CitaEncontrada], referencias: List[ReferenciaAnalizada], 
                                errores: List[str]) -> List[str]:
        """Genera recomendaciones específicas para mejorar"""
        recomendaciones = []
        
        if errores:
            recomendaciones.append(f"Corregir {len(errores)} errores de formato detectados")
        
        refs_baja_calidad = [ref for ref in referencias if ref.calidad_apa < 7]
        if refs_baja_calidad:
            recomendaciones.append(f"Revisar formato de {len(refs_baja_calidad)} referencias con calidad baja")
        
        if len(citas) == 0:
            recomendaciones.append("El documento carece de citas. Incluir respaldo bibliográfico.")
        elif len(citas) < 10:
            recomendaciones.append("Considerar incluir más citas para mayor sustento teórico")
...
"""
    ...
    if resultado.citas_encontradas:
        ...
        for cita in resultado.citas_encontradas[:10]:  # Primeras 10
            ...
            reporte += f"| {cita.autores[0]} | {cita.año} | {cita.tipo} | {tiene_ref} |\n"
    
    # Referencias problemáticas
    reporte += f"\n### 📚 Análisis de Referencias\n"
    refs_problematicas = [ref for ref in resultado.referencias_analizadas if ref.errores]
