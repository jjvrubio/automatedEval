...
def find_first_line_with_anex(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if 'anex' in norm_line:
            print(f"[find_first_line_with_anex] Posible fin de referencias en la línea {i}: {repr(line)}")
            return i
    return None
...
def extract_references_section(text):
    ...
    if ref_end is not None:
        ...
        print(f"Fin de referencias (por coincidencia parcial) en la línea {ref_end}: {repr(lines[ref_end])}")
...
def extract_apa_citations(text):
    ...
    for match in matches:
        ...
        norm_authors = norm_authors.replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u')
        ...
        if ref_start_pattern.match(line.strip()):
            if buffer:
                if len(buffer.strip()) > max_length:
                    print(f"[ADVERTENCIA] Referencia sospechosamente larga ({len(buffer.strip())} caracteres): {repr(buffer.strip())[:200]}...")
...
def extract_references_section_docx_xml(docx_path):
    ...
    if anex_after_ref:
        ...
        print(f"[XML] Fin de referencias en el párrafo {ref_end_idx} (por 'anex'): {anex_paras[anex_after_ref.index(ref_end_idx)][1]}")
    else:
        ref_end_idx = None
...
def ref_key_from_reference(ref):
    ...
    if m:
        ...
        norm_autores = norm_autores.replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u')
...
def debug_print_claves(apa_keys, citations):
    ...
    print("[DEBUG] Claves de citas encontradas (primer apellido, año):", sorted(citations))
...
def main():
    ...
    with open(os.path.join(out_dir, f"{out_base}_claves_referencias.json"), "w", encoding="utf-8") as f:
        json.dump(sorted(list(apa_keys)), f, ensure_ascii=False, indent=2)
    with open(os.path.join(out_dir, f"{out_base}_claves_citas.json"), "w", encoding="utf-8") as f:
    ...
    if not apa_references:
        print("[ADVERTENCIA] El conjunto de referencias APA está vacío antes de guardar el JSON.")
    if not non_apa_references:
        print("[ADVERTENCIA] El conjunto de referencias NO APA está vacío antes de guardar el JSON.")
    with open(os.path.join(out_dir, f"{out_base}_citas.json"), "w", encoding="utf-8") as f:
        json.dump(sorted(list(citations)), f, ensure_ascii=False, indent=2)
    with open(os.path.join(out_dir, f"{out_base}_referencias_apa.json"), "w", encoding="utf-8") as f:
        json.dump(sorted(list(apa_references)), f, ensure_ascii=False, indent=2)
    with open(os.path.join(out_dir, f"{out_base}_referencias_no_apa.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(out_dir, f"{out_base}_referencias_no_citadas.json"), "w", encoding="utf-8") as f:
        json.dump(sorted(list(not_cited)), f, ensure_ascii=False, indent=2)
    with open(os.path.join(out_dir, f"{out_base}_citas_sin_referencia.json"), "w", encoding="utf-8") as f:
