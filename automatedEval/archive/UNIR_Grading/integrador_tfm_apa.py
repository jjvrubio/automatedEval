...
from validador_apa_completo import ValidadorAPACompleto, ResultadoValidacionAPA, generar_reporte_apa_detallado
from validar_citas_openai import validar_citas_integral, extraer_referencias_automaticamente
...
class IntegradorTFMAPA:
    ...
    def evaluar_componente_bibliografico(self, texto_completo: str, archivo_tfm: str = None) -> ResultadoIntegracion:
        """
        ...
        try:
            ...
            referencias = extraer_referencias_automaticamente(archivo_tfm if archivo_tfm else texto_completo)
            logger.info(f"📖 Referencias extraídas: {len(referencias)}")
            
            # 2. Validación APA completa (local)
            resultado_apa = self.validador_apa.validar_documento_completo(texto_completo, referencias)
            
            # 3. Validación OpenAI (opcional, más profunda)
            validacion_openai = None
            if self.config.validacion_openai:
                try:
                    validacion_openai = validar_citas_integral(texto_completo, referencias)
            ...
            cumple_requisitos = self._verificar_requisitos_minimos(resultado_apa, len(referencias))
            
            # 6. Generar recomendaciones específicas
            recomendaciones = self._generar_recomendaciones_bibliograficas(resultado_apa, len(referencias))
            ...
            resultado.reporte_integrado = self._generar_reporte_integrado(resultado, validacion_openai)
            
            logger.info(f"✅ Evaluación bibliográfica completada - Puntuación: {puntuacion_bibliografica:.1f}/100")
            return resultado
    ...
    def integrar_con_tfm(self, resultado_bibliografico: ResultadoIntegracion, 
                        puntuacion_tfm_base: float) -> ResultadoIntegracion:
        """
        ...
        resultado_bibliografico.reporte_integrado = self._generar_reporte_integrado(resultado_bibliografico)
        
        logger.info(f"🎯 Puntuación final integrada: {puntuacion_final:.1f}/100")
        return resultado_bibliografico
    
    def _calcular_puntuacion_bibliografica(self, resultado_apa: ResultadoValidacionAPA, 
                                         validacion_openai: Dict = None, 
                                         num_referencias: int = 0) -> float:
        ...
        if num_referencias >= self.config.minimo_referencias:
            bonus_cantidad = min(10, (num_referencias - self.config.minimo_referencias) * 2)
        else:
        ...
        puntuacion_final = puntuacion_base + bonus_cantidad + bonus_openai + penalizacion_estricta
        
        return max(0, min(100, puntuacion_final))
    
    def _verificar_requisitos_minimos(self, resultado_apa: ResultadoValidacionAPA, 
                                    num_referencias: int) -> bool:
        ...
        if self.config.modo_estricto:
            requisitos.extend([
                len(resultado_apa.referencias_huerfanas) <= 2,  # Máximo 2 referencias no citadas
                len(resultado_apa.errores_formato) == 0,  # Sin errores de formato
            ])
        
        return all(requisitos)
    
    def _generar_recomendaciones_bibliograficas(self, resultado_apa: ResultadoValidacionAPA, 
                                              num_referencias: int) -> List[str]:
        ...
        if num_referencias < self.config.minimo_referencias:
            ...
            recomendaciones.append(f"📚 Incluir {faltantes} referencias adicionales (mínimo: {self.config.minimo_referencias})")
        ...
        if len(resultado_apa.citas_encontradas) < self.config.minimo_citas:
            ...
            recomendaciones.append(f"📖 Incluir {faltantes} citas adicionales (mínimo: {self.config.minimo_citas})")
        
        # Citas huérfanas (crítico)
        if resultado_apa.citas_huerfanas:
            recomendaciones.append(f"🚨 CRÍTICO: {len(resultado_apa.citas_huerfanas)} citas sin referencia correspondiente")
        
        # Referencias huérfanas
        if resultado_apa.referencias_huerfanas:
            if len(resultado_apa.referencias_huerfanas) > 5:
                recomendaciones.append(f"⚠️ {len(resultado_apa.referencias_huerfanas)} referencias no citadas - considerar citar o eliminar")
            else:
                recomendaciones.append(f"📝 {len(resultado_apa.referencias_huerfanas)} referencias no citadas - revisar necesidad")
        
        # Calidad APA
        if resultado_apa.puntuacion_apa < self.config.puntuacion_minima_apa:
            recomendaciones.append(f"📐 Mejorar formato APA (actual: {resultado_apa.puntuacion_apa}/100, mínimo: {self.config.puntuacion_minima_apa}/100)")
        
        # Errores específicos
        if resultado_apa.errores_formato:
            recomendaciones.append(f"🔧 Corregir {len(resultado_apa.errores_formato)} errores de formato APA")
        
        # Diversidad de fuentes
        tipos_encontrados = set(ref.tipo for ref in resultado_apa.referencias_analizadas)
        if len(tipos_encontrados) < 3:
            recomendaciones.append("🌐 Diversificar tipos de fuentes (artículos, libros, web, etc.)")
        
        return recomendaciones
    
    def _generar_reporte_integrado(self, resultado: ResultadoIntegracion, 
                                 validacion_openai: Dict = None) -> str:
...
"""
    ...
    def exportar_resultados(self, resultado: ResultadoIntegracion, ruta_salida: str):
        ...
        # 1. Reporte Markdown
        with open(directorio / f"{ruta_base}_reporte_integrado.md", 'w', encoding='utf-8') as f:
        ...
        datos_json = {
            ...
            'estadisticas': {
                ...
                'num_referencias': len(resultado.resultado_apa_detallado.referencias_analizadas),
                'citas_huerfanas': len(resultado.resultado_apa_detallado.citas_huerfanas),
                'referencias_huerfanas': len(resultado.resultado_apa_detallado.referencias_huerfanas),
                'errores_formato': len(resultado.resultado_apa_detallado.errores_formato)
            },
...
def main():
    ...
    for universidad, config in CONFIGS_UNIVERSIDAD.items():
        print(f"   - {universidad}: {config.peso_bibliografico*100:.0f}% peso bibliográfico")
    print("")
