...
try:
    ...
    from validar_citas_openai import extraer_texto_documento, seleccionar_archivo_con_dialogo
...
def prueba_completa_integracion(archivo_tfm: str = None, universidad: str = 'UNIR'):
    ...
    try:
        ...
        logger.info(f"⚙️ Configuración aplicada:")
        ...
        logger.info(f"   - Validación OpenAI: {'✅' if config.validacion_openai else '❌'}")
        ...
        resultado = integrador.evaluar_componente_bibliografico(texto_completo, archivo_tfm)
        ...
        resultado_final = integrador.integrar_con_tfm(resultado, puntuacion_tfm_simulada)
        ...
        logger.info(f"📚 Puntuación Bibliográfica: {resultado_final.puntuacion_bibliografica:.1f}/100")
        logger.info(f"📖 Puntuación APA: {resultado_final.puntuacion_apa}/100")
        logger.info(f"✅ Cumple requisitos: {'SÍ' if resultado_final.cumple_requisitos_minimos else 'NO'}")
        ...
        ruta_salida = Path(archivo_tfm).parent / f"evaluacion_integrada_{universidad.lower()}"
        ...
        if resultado_final.recomendaciones_bibliograficas:
            ...
            for i, rec in enumerate(resultado_final.recomendaciones_bibliograficas[:3], 1):
                logger.info(f"   {i}. {rec}")
        ...
        logger.info(f"   - integracion_tfm_apa.log")
