...
def extract_body_text(docx_path):
    ...
    try:
        with zipfile.ZipFile(docx_path, "r") as docx_zip:
            ...
            for element in tree.xpath("//w:p", namespaces={"w": "http://schemas.openxmlformats.org/wordprocessingml/2006/main"}):
                paragraph_text = "".join(
                    t.text for t in element.xpath(".//w:t", namespaces={"w": "http://schemas.openxmlformats.org/wordprocessingml/2006/main"}) if t.text
                ).strip()
                if paragraph_text:
...
def extract_last_bibliography_section(body_text, language):
    """Extract text starting from the last occurrence of the bibliography section."""
    last_index = -1
    ...
    if last_index != -1:
        # Extract all paragraphs from the last occurrence onward
        return body_text[last_index:]
    else:
        return []
...
def save_bibliography_to_markdown(docx_path, output_path, language):
    ...
    if bibliography_content:
        ...
        print(f"No bibliography section found in the document for language: {language}.")
...
def main():
    ...
    if docx_path:
        ...
        if language == "unknown":
            print("Could not determine the language. Please ensure the document contains either 'Referencias bibliográficas' or 'References'.")
        else:
            print(f"Detected language: {language.capitalize()}")

            # Output path for Bibliography Markdown file
            bibliography_md_path = docx_path.replace(".docx", f"_{language}_bibliography.md")
